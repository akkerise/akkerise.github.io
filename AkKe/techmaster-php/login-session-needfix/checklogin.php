...
//    }
    if (isset($_POST['username']) && isset($_POST['password'])){
        ...
        if (!preg_match($pattern, $_SESSION['username'] , $match)){
//            var_dump($match);
            $_SESSION['error_username'] = " Sai định dạng username !";
            header("Location: login.php");
            $flag = 0;
            exit();
        }
        if (!preg_match($pattern, $_SESSION['password'] , $match)){
            $_SESSION['error_password'] = " Sai định dạng password !";
            header("Location: login.php");
//            var_dump($match);
            $flag = 0;
            exit();
        ...
        if ($flag == 1){
            if ($_SESSION['username'] != $user['username']){
              $flag = 0;
              $_SESSION['error_username'] = "Nhập sai username";
              header("Location: login.php");
              exit();
            }
            if ($_SESSION['password'] != $user['password']){
              $flag = 0;
              $_SESSION['error_password'] = "Nhập sai password";
              header("Location: login.php");
              exit();
            }
