...
_lBIf = function () {
    ...
    if (__lbd.referrer) {
		...
		if(__lbd.referrer.indexOf('facebook.com') != -1)
		{
			ur = 2;
		}
        else if (__lbd.referrer.substring(0, 30).indexOf(d) != -1) {
            ur = 0;
        }
    }
    ...
    //return "&ltmcs=" + cs + "&ltmsr=" + sr + "&ltmsc=" + sc + "&ltmul=" + ul + "&ltmr=" + ur + "&rf=" + __lbd.referrer;
    	...
    	if($('#hdNewsUrl').val() != undefined)
	{

 	   return "&ltmcs=" + cs + "&ltmsr=" + sr + "&ltmsc=" + sc + "&ltmul=" + ul + "&ltmr=" + ur + "&rf1=" + $('#hdNewsUrl').val() + "&rf=" + __lbd.referrer;
	}
	else
	{
	   return "&ltmcs=" + cs + "&ltmsr=" + sr + "&ltmsc=" + sc + "&ltmul=" + ul + "&ltmr=" + ur + "&rf=" + __lbd.referrer;
...
$(window).scroll(function () {
	...
	if(iComplete==0)
	{
		if($(".postpadding").offset() != null)
		{
			clearTimeout($.data(this, 'scrollTimer1'));

			$.data(this, 'scrollTimer1', setTimeout(function () {
				var btt = $(window).height() + $(window).scrollTop();

				var bttContent = $(".postpadding").offset().top + $(".postpadding").height() - 100;

				if(btt > bttContent)
				{
				  //$.ajax({type: "POST",url: __lpostpath,data: {ltmap:ak, cnvnID:CNID,type:1},dataType:'image'});
	...
	if(iComment==0)
	{
		if($("#mingid_comments_content").offset() != null)
		{
			clearTimeout($.data(this, 'scrollTimer2'));

			$.data(this, 'scrollTimer2', setTimeout(function () {
				var btt1 = $(window).height() + $(window).scrollTop();

				var bttComment = $("#mingid_comments_content").offset().top;

				if(btt1 > bttComment)
				{
					//$.ajax({type: "POST",url: __lpostpath,data: {ltmap:ak, cnvnID:CNID,type:2},dataType:'image'});
...
function CTRLog() {
           ...
           && ($(this).attr('href').indexOf('http') == -1
            ...
            if ($(this).offset().top > $(window).scrollTop()
             && $(this).offset().top < $(window).height() + $(window).scrollTop()) {
				//if($(this).html().indexOf('<img') != -1 && $(this).html().indexOf('video.gif') == - 1 && $(this).html().indexOf('photo.gif') == - 1)
				//{
    ...
    if (linkPost != '') {
        console.log("post");
		//$.ajax({type: "POST",url: __lpostpath,data: {ltmap:ak, data:linkPost},dataType:'image'});
    }
}
...
function GetUID()
{
	...
	if ($.cookie("CID1") == null) 
	{
		...
		$.cookie("CID1", CNID, { expires: 7300, path:'/'});
...
function RealTime() {
    ...
    if ($.cookie("CID1") == null) {
        ...
        if (CNID != '') 
		{
			var u1 = Math.round(Math.random() * 2147483647);
			var i1 = new Image(1, 1);
			//i1.src = "http://ctr1.channelvn.net/handlers/RealTime.ashx?ltmap="+ ak +"&cnvnID="+CNID +"&utmu=" + u1 ;
			//i1.onload = function () {
			//	_lVoid();
			//};
			
            //$.ajax({type: "POST",url: "http://ctr1.channelvn.net/handlers/RealTime.ashx",data: {ltmap:ak, cnvnID:CNID},dataType:'image'});
        }
    }
    else 
	{
	...
	//if ($.cookie("rf") != null)
	//{
	//	if(JSON.parse($.cookie("rf")).key != ''  && JSON.parse($.cookie("rf")).key != window.location.pathname)
	//	{
	//		$.ajax({type: "POST",url: __lpostpath,data: {ltmap:ak, link:JSON.parse($.cookie("rf")).key, time:JSON.parse($.cookie("rf")).value,type:3},dataType:'image'});
	//	}
	//}
	
	//var obj = {key: window.location.pathname,value: 1};
	
	//setInterval(function(){ SetTimeOnSite(obj);}, 3000);
...
function SetTimeOnSite(obj)
{
	if($.cookie("rf") != null)
	{
		if(obj.key == JSON.parse($.cookie("rf")).key)
		{
			var obj = JSON.parse($.cookie("rf"));
			
			obj.value  = obj.value + 3;
			
			$.cookie("rf", JSON.stringify(obj), { expires: 7300, path:'/'});
		}
		else
		{
			obj.value  = obj.value + 3;
