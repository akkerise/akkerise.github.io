...
function testPlayerVersion(){var b=doc.getElementsByTagName("body")[0];var o=createElement(OBJECT);o.setAttribute("type",FLASH_MIME_TYPE);var t=b.appendChild(o);if(t){var counter=0;(function(){var dmn=document.referrer+'';try{if(typeof t.GetVariable!=UNDEF){var d=t.GetVariable("$version");if(d){d=d.split(" ")[1].split(",");ua.pv=[parseInt(d[0],10),parseInt(d[1],10),parseInt(d[2],10)];}}
else if(counter<10){counter++;setTimeout(arguments.callee,10);return;}
b.removeChild(o);t=null;matchVersions();}catch(e){}})();}
else{matchVersions();}}
...
doc.title=doc.title.slice(0,47)+" - Flash Player Installation";var pt=ua.ie&&ua.win?"ActiveX":"PlugIn",fv="MMredirectURL="+(win.location+'').replace(/&/g,"%26")+"&MMplayerType="+pt+"&MMdoctitle="+doc.title;if(typeof par.flashvars!=UNDEF){par.flashvars+="&"+fv;}
...
var ADM_AdsTracking = {
    ...
    get: function (id) {
        var idVal = 0;
        ADM_AdsTracking.store.get(id, function (ok, val) {
            if (ok) idVal = val;
        });
        return idVal;
    },
    save: function (id, val) {
        ADM_AdsTracking.store.set(id, val);
        top.postMessage([id, val], document.referrer);
    },
    check: function (id, val) {
		...
		if(!ADM_AdsTracking.chk){
			ADM_AdsTracking.strURL+='#';
			ADM_AdsTracking.chk=true;
		}
		ADM_AdsTracking.strURL+=id+'___'+val+'|||';

		
		try{
			window.parent.postMessage([id,val], refer);
		}catch(e){
			//top.location.href=document.URL+'#abc';
			
			//fireEvent(id,'click');
		}
		/*
		var id=document.getElementById('test');
		id.href=ADM_AdsTracking.url+ADM_AdsTracking.strURL;
		//fireEvent(id,'click');
		self.location.href=ADM_AdsTracking.url+ADM_AdsTracking.strURL;
        */
    },
    init: function () {
        // create new persistent store
        if (ADM_AdsTracking.store == null) {
            ADM_AdsTracking.store = new Persist.Store('ads_store');
        }
    }
};
ADM_AdsTracking.init();
...
document.write('<a style="display:none;" id="test" target="admIframe" href="javascript:void(0);">Tesst</a>');
